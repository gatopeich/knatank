...
# FIX Multiple collision -> ValueError: list.remove(x): x not in list.

import os, sys, pygame, time, math, bisect
...
class Sprite:
    list = []
    def __init__(self):
	bisect.insort(Sprite.list,self)
    def __lt__(self,other):
        return self.y < other.y
    def dissapear(self):
	try:
	    Sprite.list.remove(self)
	except:
    ...
    def draw(self): pass
    def __str__(self):
	return self.__class__.__name__+(" at "+str(self.box) if hasattr(self,'box') else '')
    ...
    def collideall():
	collisions = []
	objects = [o for o in Sprite.list if hasattr(o,'box')]
        for o1 in objects:
	    for o2 in objects:
		if o1 != o2 and o1.box.colliderect(o2.box):
		    print o1,"hit",o2
		    o1.collision(o2)
...
class Tank(Sprite):
	...
	w = self.sx
	self.box = pygame.Rect(x-w/2, y-w/2, w, w)
	...
	if self.readyamo and pygame.mouse.get_pressed()[0]:
	    ...
	    vx, vy = 10*dx/distance, 10*dy/distance
...
class Trail(Sprite):
    def __init__(self,color,x,y):
        ...
        Sprite.__init__(self)
    def draw(self):
        draw.circle(screen, self.color, (self.x,self.y/2-BULLET_HEIGHT) , 3, 1)
...
class Explosion(Sprite):
	...
	sound_explosion.play()
        ...
        Sprite.__init__(self)
    def draw(self):
        draw.circle(screen, red, (self.x, self.y/2-BULLET_HEIGHT), self.size)
        self.size -= 5
	if 2 > self.size:
	    self.dissapear()
...
class Bullet(Sprite):
	...
	self.bounces = 1
	self.box = pygame.Rect(x-1,y-1,3,3)
        Sprite.__init__(self)
    def update(self):
        self.fx += self.vx
        self.fy += self.vy
	self.y = int(self.fy>>8)
	self.box.center = int(self.fx>>8), self.y
    def collision(self,other):
	Explosion(10,self.box.center)
	self.dissapear()
    def draw(self):
...
def game():
    tanks = ( Tank(blue,field_width/3, field_height/3),
              Tank(rosa,field_width*2/3, field_height/3),
